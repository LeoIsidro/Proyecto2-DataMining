"""
streaming.py — Parte V: Minería de Flujos de Datos (Data Stream Mining)

Contiene las tres técnicas requeridas por la consigna:

1. Ventanas deslizantes (estadisticas_por_ventana, metricas_globales_por_ventana)
2. Conteo aproximado — Count-Min Sketch (clase CountMinSketch)
   Ref: Cormode, G., & Muthukrishnan, S. (2005). An improved data stream
   summary: the count-min sketch and its applications. Journal of
   Algorithms, 55(1), 58-75.
3. Técnica adicional (obligatoria) — DGIM, conteo de 1's en ventana
   deslizante binaria (clase DGIM)
   Ref: Datar, M., Gionis, A., Indyk, P., & Motwani, R. (2002).
   Maintaining stream statistics over sliding windows. SIAM Journal on
   Computing, 31(6), 1794-1813.
4. Técnica adicional (plus / extra) — HyperLogLog, estimación de
   cardinalidad (elementos distintos) con memoria O(log log N)
   (clase HyperLogLog)
   Ref: Flajolet, P., & Martin, G. N. (1985). Probabilistic counting
   algorithms for data base applications. Journal of Computer and
   System Sciences, 31(2), 182-209.
   Flajolet, P., Fusy, É., Gandouet, O., & Meunier, F. (2007).
   HyperLogLog: the analysis of a near-optimal cardinality estimation
   algorithm. Proceedings of AofA, 137-156.

Todas las estructuras están pensadas para procesar el stream elemento a
elemento (o bit a bit), con memoria acotada e independiente del tamaño
total del stream, respetando la restricción central de streaming: no se
puede releer ni almacenar el historial completo.
"""
...
# 4. HYPERLOGLOG (Flajolet & Martin, 1985; Flajolet et al., 2007)
# Técnica "plus" adicional: estimación de cardinalidad (# de elementos
# distintos) en un stream, con memoria O(log log N) en vez de O(N).

class HyperLogLog:
    """Estima el número de elementos DISTINTOS de un stream (cardinalidad)
    sin almacenar el conjunto de elementos vistos.

    Idea: se hashea cada elemento a un entero de 32 bits. Los primeros
    `p` bits eligen uno de `m = 2**p` "registros"; en cada registro se
    guarda el mayor "rank" observado (posición del primer bit en 1,
    contando desde la izquierda, en los bits restantes). Cuanto más
    elementos distintos se han visto, más probable es observar rachas
    largas de ceros -> se usa una media armónica de 2**rank sobre los m
    registros para estimar la cardinalidad.
    """

    def __init__(self, p: int = 11, seed: int = 42):
        if not (4 <= p <= 20):
            raise ValueError("p debe estar entre 4 y 20 (m = 2**p registros)")
        self.p = p
        self.m = 1 << p
        ...
        # constante de corrección de sesgo (bias) estándar de HLL
        if self.m == 16:
            self.alpha = 0.673
        elif self.m == 32:
            self.alpha = 0.697
    ...
    def _hash32(self, x) -> int:
        s = f"{self.seed}_{x}".encode("utf-8")
        digest = hashlib.md5(s).digest()
        return int.from_bytes(digest[:4], "big")  # entero de 32 bits

    @staticmethod
    def _rank(w: int, n_bits: int) -> int:
        """Posición (1-indexada) del primer bit en 1 de `w`, leyendo
        `n_bits` bits desde el más significativo. Si w == 0, rank = n_bits+1."""
        if w == 0:
            return n_bits + 1
        rank = 1
        mask = 1 << (n_bits - 1)
    ...
    def add(self, x) -> None:
        """Procesa un elemento del stream (inserción), un elemento a la vez."""
        h = self._hash32(x)
        n_bits = 32 - self.p
        idx = h >> n_bits                    # p bits más significativos -> registro
        resto = h & ((1 << n_bits) - 1)       # bits restantes -> se mide el rank
        rank = self._rank(resto, n_bits)
        if rank > self.registers[idx]:
            self.registers[idx] = rank
        self.n_procesados += 1

    def estimate(self) -> float:
        """Cardinalidad estimada (# de elementos distintos)."""
        indicador = sum(2.0 ** (-r) for r in self.registers)
        E = self.alpha * (self.m ** 2) / indicador

        if E <= 2.5 * self.m:
            # small-range correction: linear counting si hay registros vacíos
            V = self.registers.count(0)
            if V > 0:
                return self.m * math.log(self.m / V)
        return E

    def error_bound_relativo(self) -> float:
        """Error estándar relativo típico de HLL: ~= 1.04 / sqrt(m)."""
        return 1.04 / math.sqrt(self.m)

    def memory_bytes(self) -> int:
        """Memoria aproximada: 1 byte por registro (holgado; en implementaciones
        reales cada registro cabe en ~5-6 bits empaquetados)."""
        return self.m
